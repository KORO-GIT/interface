class DamageText4Wnd extends SublimityItem;
...
function OnLoad()
{
    ...
    while(i <= 30)
    {
        ...
        while(ii < 10)
        {
            m_Digits[C + ii] = TextureHandle(GetHandle(((("DamageText4Wnd.DamageText4" $ string(i)) $ "Wnd.Digit") $ string(i)) $ string(ii + 1)));
            ++ii;
        }
        ++i;
    }
...
function HandleRestart()
{
    ...
    while(i <= 30)
    {
        HideWindow(("DamageText4Wnd.DamageText4" $ string(i)) $ "Wnd");
        ++i;
    }
    return;
}
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if(Index == "361")
                                {
                                    ShowIndicator(15, 0, 50, false);                                    
                                }
                                else
                                {
                                    // End:0x188
                                    if((Index == "45") || Index == "95")
                                    {
                                        ParseString(a_Param, "Param1", Param1);
                                        ShowIndicator(6, int(Param1), 1000, false);
                                    }
                                }
                            }
                        }
...
function ShowIndicator(int Type, int Value, int nGroupThreshold, bool bGrouped)
{
    ...
    if(nGroupThreshold > 0)
    {
        ...
        Class'NWindow.UIAPI_WINDOW'.static.KillUITimer("DamageText4Wnd", -1 * Type);
        Class'NWindow.UIAPI_WINDOW'.static.SetUITimer("DamageText4Wnd", -1 * Type, nGroupThreshold);        
    }
    else
    {
        m_LastUsedContainer++;
...
function ShowAndAnimate(int Type, int Container, int Value, bool bGrouped)
{
    ...
    WindowName = ("DamageText4Wnd.DamageText4" $ string(Container)) $ "Wnd";
    Class'NWindow.UIAPI_WINDOW'.static.KillUITimer("DamageText4Wnd", Container);
    Class'NWindow.UIAPI_WINDOW'.static.KillUITimer("DamageText4Wnd", -100 * Container);
    ...
    Class'NWindow.UIAPI_WINDOW'.static.SetUITimer("DamageText4Wnd", Container, 100);
    return;
}
...
function OnTimer(int Container)
{
    ...
    if(Container > 0)
    {
        ...
        {
            Class'NWindow.UIAPI_WINDOW'.static.SetUITimer("DamageText4Wnd", -100 * Container, 100);            
        }
        else
        {
            Class'NWindow.UIAPI_WINDOW'.static.SetUITimer("DamageText4Wnd", -100 * Container, 100);
...
function TimerFadeOut(int Container, int Duration, int Steps)
{
    ...
    while(i <= Steps)
    {
        Class'NWindow.UIAPI_WINDOW'.static.SetUITimer("DamageText4Wnd", (-100 * Container) - (100 - (i * C)), i * D);
        i++;
    }
    return;
}
